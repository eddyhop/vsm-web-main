...
import {getRolelists, delRole, deviceSelect, userInfo} from "@/services/index.js";
export default {
  components: {
    rolePop,
    roleSetting
  ...
  methods: {
    ...
    addRole() {
      this.isShow = true;
      this.popData = {
        name: this.$t("Role.addRolePermission"),
        type: "add"
      };
    },
    // 角色编辑
    ...
    editRole(val) {
      this.isShow = true;
      this.popData = {
        name: this.$t("Role.editRolePermission"),
        type: "edit",
    ...
    // Delete the role
    cancelRole(row) {
      var show93 = localStorage.getItem("show93");
      if(show93 == 'true'){
        ...
        this.$confirm(this.$t("Role.hint3")+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue93\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", this.$t("Role.hint"), {
        ...
        .then(() => {
          delRole(row.id).then(res => {
            if (res.code === 10000) {
              ...
              this.getRolelist();
              localStorage.setItem("show93",localStorage.getItem("checkedValue93"));
            } else {
              this.$message({
                type: "error",
                message: res.message
