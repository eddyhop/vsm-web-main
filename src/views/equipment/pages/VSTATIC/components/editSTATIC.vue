<template>
  <div class="deitSTATIC isShow" ref="deitSTATIC">
    <div :class="`context ${lang=='en'?'contextEn':''}`">
      <h4 class="title">
        <span>{{isEdit ? $t('vsr1000.editSR') : '查看'}} </span>
      ...
      <div class="event">
        ...
        >
          ...
          <el-form-item :label="$t('vsr1000.name')" prop="interName">
            <el-input v-model="editData.interName"  :placeholder="$t('vsr1000.name')" ></el-input>
          </el-form-item>

          <el-form-item :label="$t('vsr1000.describe')" prop="des">
            <el-input v-model="editData.des" :placeholder="$t('vsr1000.describe')" ></el-input>
          </el-form-item>

          <el-form-item :label="$t('vsr1000.destinationNetwork')" prop="ip">
            <el-input v-model="editData.ip" @blur="zhuan" :placeholder="$t('vsr1000.destinationNetwork')" ></el-input>
          </el-form-item>

          <el-form-item :label="$t('vsr1000.nextHop')" prop="nextHopIp">
            <el-input v-model="editData.nextHopIp" :placeholder="$t('vsr1000.nextHop')"></el-input>
          </el-form-item>

          <el-form-item :label="$t('vsr1000.distance')" prop="dis">
            <el-input v-model="editData.dis" :placeholder="$t('vsr1000.distance')"></el-input>
          </el-form-item>
...
import {
  ...
  checkRepeatNextIp, nameExistStatic //下一跳重复性校验
...
export default {
  ...
  data() {
    var checkName = (rule,value,callback) => {
      if(value){
        if(this.editData.oldInterName!=value){
          nameExistStatic({checkName:value,deviceId:JSON.parse(this.$route.query.devData).deviceId}).then(res=>{
            if(res.code==10000){
              callback()
            }else{
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      let data = {
        ...
        interName:this.editData.interName,
        tenantId: JSON.parse(this.$route.query.devData).tenantId
      };
    ...
    async dataInit(row, type) {
      ...
      this.editData.interName = row.interName;
      this.editData.oldInterName = row.interName;
      this.editData.des = row.des;
