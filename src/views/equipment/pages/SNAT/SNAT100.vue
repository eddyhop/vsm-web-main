<template>
  ...
  <div class="SNAT">
    <div :class="lang=='en'? 'contentEn content' : 'content'">
      ...
      >
        <el-form-item :label="$t('vsr100.inIP')" prop="inIp">
          <el-input
            v-model="ruleForm.inIp"
            :placeholder="$t('vsr100.lan.enterContent')"
            size="small"
          ></el-input>
        </el-form-item>
        <el-form-item :label="$t('vsr100.outIP')" prop="outIp">
          <el-input
  ...
  methods: {
    ...
    removeSnat() {
      if (this.ruleForm.id.length) {
        var show55 = localStorage.getItem("show55");
        if(show55 == 'true'){
          ...
          this.$confirm(this.$t('newModel.msg17')+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue55\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", {
              ...
              if (res.success) {
                snatDel({ ids: this.ruleForm.id }).then((res) => {
                  if (res.code === 10000) {
                    ...
                    this.$refs.form.resetFields();
                    localStorage.setItem("show55",localStorage.getItem("checkedValue55"));
                  } else if (res.code !== 10000) {
                    this.$message({
                      type: "warning",
                      message: res.message,
    ...
    dataInit() {
      snatGet({ deviceId: this.deviceId }).then((res) => {
        if (res.code === 10000 && res.result != null) {
          ...
          this.ruleForm.inIp = res.result.inIp;
        }
      });
    },
  },
