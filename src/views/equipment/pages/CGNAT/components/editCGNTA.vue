<template>
  <div class="addCGNAT" v-show="isShow2">
    <div class="context">
      <h4 class="title">
        <span>CGNAT{{$t('facility.edit')}}</span>
      ...
      <div class="event">
        ...
        >
          ...
          <div class="submit">
            <button class="save" type="primary" @click.prevent="submitForm('ruleForm')">{{$t('facility.save')}}</button>
...
import { editCgnat, getProtoclItem, getCgnatAddrDetail } from "@/services";
export default {
  props: {
    isShow2: {
      type: Boolean
  ...
  methods: {
    ...
    tabsDrop() {
      let el = document.querySelectorAll(".tabsList8 .el-tabs__nav")[0];
      const _this = this;
      Sortable.create(el, {
    ...
    dataInit() {
      getProtoclItem().then(res => {
        this.protoclList = res.result;
      });
    },
    async editData(row) {
      let data = await getCgnatAddrDetail({ cgnatId: row.id });
      console.log(data);
      this.ruleForm.editableTabs = data.result.map((item, index) => {
        return {
          title: "静态地址映射" + (index + 1),
          name: index + 1 + "",
          content: item
        };
      });
      ...
      this.ruleForm.icmpTimeOut = row.icmpTimeOut;
      this.ruleForm.id = row.id;
      this.ruleForm.inAddr = row.inAddr;
      this.ruleForm.tcpTimeOut = row.tcpTimeOut;
      this.ruleForm.udpTimeOut = row.udpTimeOut;
      this.tabIndex = data.result.length;
      this.editableTabsValue = "1"
      this.ruleForm.ipAddress = row.outAddr.split(",").map(item => {
        return {
          outAddr: item,
          key: Date.now()
        };
