<template>
  <div class="editStatic">
    <div class="context">
      <div class="title">
        {{$t('subscription.addressTranslation.addPortForwarding')}}
      ...
      <div class="item" :style="lang == 'en' ? '' : 'marginLeft:70px'">
        ...
        >
          ...
          <el-form-item :label="$t('subscription.addressTranslation.externalPort')" prop="outPort">
            <div class="item-ipt">
              <el-input v-model.number="form.outPort" size="small" :placeholder="$t('subscription.addressTranslation.externalPort')"></el-input>
            </div>
          </el-form-item>
          ...
          <el-form-item :label="$t('subscription.addressTranslation.internalPort')" prop="locpalPort">
            <div class="item-ipt">
              <el-input v-model="form.locpalPort" size="small" :placeholder="$t('subscription.addressTranslation.internalPort')"></el-input>
            </div>
          </el-form-item>
...
<script>
import {getDict, snatGet, dnatCheckName, dnatSave, getLogicPortForRole} from "@/services";
export default {
  data() {
  ...
  methods: {
    close() {
      this.$parent.addPortForwardingShow = false;
    ...
    GetSNAT() {
      snatGet({ deviceId: this.deviceId }).then((res) => {
        if (res.code === 10000 && res.result != null) {
          ...
          this.form._outIp = res.result.outIp;
          this.form.outIp =  "0.0.0.0/0",
          this.form.snat = res.result.outInterfaceId;
        }
      });
