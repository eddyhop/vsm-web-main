    ...
    <div class="vsrPanCon">
      <dl class="hint">
        ...
        <dd>
          ...
          <p class="devTime">
            ...
            <!-- <span>{{$t('vsnOperaions.timeFormat')}}：{{secondsFormat(keepTime)}}</span> -->
            <!-- <span>{{$t('vsr1000.sustainOnLine')}}：{{getTime(startTime)}}</span> -->
          </p>
        </dd>
        <div class="operate">
          <button class="add" v-control @click="goOspfRoutesPage(deviceName)">
            {{ $t("Monit.showOspfRouts") }}
          </button>
        </div>
      </dl>
      ...
      <ul class="showList">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <li class="slItem lastItem">
              <h5 style="padding-bottom: 10px">{{ $t("Monit.ospfRoutes") }}</h5>
              <p class="flow">
                <b>{{ routerNum }}</b>
              </p>
            </li>
      ...
      <ospfStatus />
      <ospfRoutes
        :statisticsName="deviceName"
        :statisticsType="deviceName"
        ref="ospfRoutes"
        v-if="ospfRoutesShow"
      ></ospfRoutes>
    </div>
...
<script>
import ospfStatus from "./components/ospfStatus";
import ospfRoutes from "./components/ospfRoutes";
import {
  deviceBaseInfo,
...
export default {
  ...
  methods: {
    ...
    getDevDataInit(params) {
      ospfDeviceBaseInfo(params).then((res) => {
        if (res.code === 10000) {
          this.deviceName = res.result.deviceName;
          this.deviceSeries = res.result.deviceSeries;
          this.neighborsNum = res.result.neighborsNum;
          this.routerNum = res.result.routerNum;
    ...
    goOspfRoutesPage(deviceName) {
      this.statisticsName = this.deviceName;
      this.statisticsType = this.deviceName;
      //this.$refs.moreRoutes.initData(deviceName);
      this.ospfRoutesShow = true;
    },
  },
  created() {
