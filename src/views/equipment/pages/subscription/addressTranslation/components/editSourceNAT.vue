<template>
  <div class="addStatic">
    <div class="context">
      <div class="title">
        {{$t('subscription.addressTranslation.editSourceNAT')}}
      ...
      <div class="item" :style="lang=='en'?'':'marginLeft:70px'">
        ...
        >
          ...
          <!-- <el-form-item :label="$t('subscription.addressTranslation')" prop="outIp">
            <div class="item-ipt">
...
import {
  setLStatic,
  snatGet, getLogicPortForRole, snatSave
} from "@/services";
export default {
  data() {
    return {
  ...
  methods: {
    close() {
      this.$parent.editSourceNATShow = false;
    ...
    dataInit() {
      snatGet({ deviceId: this.deviceId }).then((res) => {
        if (res.code === 10000 && res.result != null) {
          ...
          this.form.outInterfaceId = res.result.outInterfaceId;
          this.form.outIp = res.result.outIp;
        }
      });
    },
    getOutIpSelect() {
