               ...
               <el-form :model="form" :rules="rules" ref="form" label-width="120px" @submit.native.prevent>
                   <el-form-item :label="$t('UserMan.Username1')" prop="name">
                    ...
                    <el-form-item :label="$t('UserMan.password1')" prop="pass">
                        <el-input v-model="form.pass" :placeholder="$t('UserMan.hint4')" show-password size="small"></el-input>
                    </el-form-item>
...
<script>
import {addControlUser} from '@/services'
export default {
    data() {
        return {
            form:{
            ...
            rules:{
                name:[
                    { required: true, message: this.$t('UserMan.hint6'), trigger: 'blur' },
                    { pattern: /^[a-zA-Z][a-zA-Z0-9]{5,32}$/, message: this.$t('UserMan.hint2') }
                    ],
                pass:[
                    { required: true, message: this.$t('UserMan.hint4'), trigger: 'blur' },
                    { pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[^]{8,}$/, message: this.$t("UserMan.hint12") }
                ],
                value:[{required:true,message:this.$t('UserMan.hint9'),trigger:'change'}]
            }
        }
    },
    methods:{
