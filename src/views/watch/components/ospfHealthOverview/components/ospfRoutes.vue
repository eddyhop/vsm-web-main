<template>
  <div class="addACL">
    <div class="context">
      <h4 class="title">
        <span>{{ $t("Monit.showOspfRouts") }}: {{ statisticsName }}</span>
...
<script>
import { getNaaSOspfNeighbours,ospfNaaSRoutesPing,getOspfRoutesPing, } from "@/services";
...
export default {
  ...
  methods: {
    ...
    refresh() {
      console.log("refresh");
      this.$parent.ospfRoutesShow = false;
    },
    close() {
      this.$parent.ospfRoutesShow = false;
      ...
      this.$refs.formTs.validate((valid) => {
        if (valid) {
          ...
          this.getRid();
              ...
              ospfNaaSRoutesPing(param).then((res) => {
                if (res.code == 10000) {
                  this.setTimer();
                }
              });
