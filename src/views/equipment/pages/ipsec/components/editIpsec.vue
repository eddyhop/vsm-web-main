...
<template>
  <div class="editIpsec">
    <div class="context">
      <h4 class="title">
        <span>{{$t('vsr1000.edit')}} IPSEC</span>
        ...
        <el-form :inline="true" :model="formInline" :rules="rules" ref="form" class="demo-form-inline" label-width="120px">
          ...
          > 
            <span class="upload">
              ...
              <input type="file" ref="editFile" class="file"> 
            </span>
...
export default {
  ...
  data() {
    var checkName = (rule, value, callback)=>{
      ipSecCanNameUse({ verifyName:value,verifyScope:this.deviceId,verifyId:this.id }).then(res=>{
        if(res.code==10000){
          callback()
        }else{
  ...
  methods: {
    save(formName){
      ...
      var file = this.$refs.editFile.files[0]
