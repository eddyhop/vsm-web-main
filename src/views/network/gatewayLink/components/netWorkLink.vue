...
export default {
  data() {
    ...
    var edgeRule = (rule, value, callback) => {
      if (value) {
        sceneRepeatIp({ ip: value, deviceId: this.ruleForm.edgeId }).then(
          (res) => {
            if (res.result) {
              callback();
            } else {
    ...
    return {
      ...
      rules: {
        ...
        cliAddr: [
          ...
          // { validator: edgeRule, trigger: "blur" },
        ],
      },
    };
  },
      ...
      sceneNodeGetVpnDetails({ sceneDeviceId: row.sceneDeviceId }).then(
        (res) => {
          if (res.code === 10000) {
            ...
            this.ruleForm.edgeId = res.result.edgeId;
