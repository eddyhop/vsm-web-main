...
<!--          <div class="chartItem">
            <button
              ...
              @click="addGroup(item.chartsArr[1].position)"
              v-control
            >{{$t('vsr1000.setGroup')}}</button>
            &lt;!&ndash; <div class="chart" id="chart2"></div> &ndash;&gt;
            <charts :id="item.domId.lossAndDelay" :data="item.chartsArr[1]" />
          </div>-->
        </div>
...
import {
  ...
  deviceTunnelNetworkFlowChart, userInfo,
} from "@/services";
export default {
  props: {},
  components: {
  ...
  //     deviceId: this,
  //     deviceType:
  //       JSON.parse(sessionStorage.getItem("devData")).deviceType == "vsr1000"
  //         ? "0001"
  //         : this.devType == "vsr100"
  //         ? "0002"
  ...
  methods: {
    ...
    dataInit(params, val) {
      ...
      deviceTunnelNetworkFlowChart(params).then((res) => {
      ...
      data.forEach((item) => {
        if (item.name === "flow") {
          if (item.time != null && item.time.length) {
            ...
            arr.push(
              this.portDataChangeNoTime(mockArr,
                item.position,
                item.isFavorite,
                this.$t("vsr1000.sent"),
                this.$t("vsr1000.received")
              )
            );
          }
    ...
    finedMaxSize(val) {
      ...
      return arr[a];
    }, // 是每一个数据换算到最大单位 Is each data conversion to the maximum unit
       // Each data conversion to the maximum unit Is each data conversion to the maximum unit
    changeMaxSize(val) {
      if (val === 0) return val;
      let arr = ["b", "Kb", "Mb", "Gb", "Tb", "Pb", "Eb", "Zb", "Yb"];
      let a = 0;
