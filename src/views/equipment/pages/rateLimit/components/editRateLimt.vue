<template>
  <!-- 新增速度限制 -->
  <div class="editRateLimt">
    <div :class="lang=='en' ? 'contentEn content' : 'content'">
      <h4 class="title">
        <span>{{$t('newModel.edit')+$t('newModel.vsr100.rateLimitation')}}</span>
...
export default {
  ...
  methods: {
    ...
    dataInit(row) {
      ...
      this.ruleForm.download = row.download;
      this.ruleForm.upload = row.upload;
      this.ruleForm.descprition = row.descprition == null ? '' : row.descprition
