<template>
  <!-- 新增端口映射 -->
  <div class="addPortMapping">
    <div class="content">
      <h4 class="title">
        <span>{{$t('newModel.add')+$t('newModel.vsr100.portMapping')}}</span>
...
<script>
import { protmapAdd, getDict, protmapCheckExtPort } from "@/services";
export default {
  data() {
    ...
    var extPort = (rule, value, callback) => {
      if (value) {
        protmapCheckExtPort({devId:this.deviceId,extPort:value}).then(res=>{
          if(res.success){
            callback()
          }else{
  ...
  methods: {
    ...
    getProtoList() {
      getDict({
        ...
        if (res.code === 10000) {
          this.protoList = res.result.sort((a,b)=>a.sortOrder-b.sortOrder)
        }
      });
    }
  }
