<template>
  <div class="editQOS">
    <div class="context">
      <h4 class="title">
        <span>{{ruleForm.name}}{{$t("vsr1000.details")}}</span>
      ...
      <div class="event">
        ...
        >
          ...
          <el-form-item :label="$t('vsr1000.describe')" prop="description" class="formItem">
            <el-input type="textarea" v-model="ruleForm.description" placeholder disabled></el-input>
          </el-form-item>
          ...
          <div class="tabsCon">
            <el-tabs v-model="editableTabsValue" type="card" closable class="tabsList4">
              ...
              >
                ...
                >
                  <el-input
                    v-model="item.content.peakBurst"
                    @change="inpIndex(index)"
                    placeholder
                    disabled
                  ></el-input>
...
export default {
  ...
  methods: {
    close() {
      this.$parent.qosDetailsShow = false;
    },
    tabsDrop() {
    ...
    async dataInit(val) {
      ...
      // //数据初始化
      // Data initialization
      let qosData = await comGetQosDetails({ qosId: val });
      ...
      if (qosToAdd.code === 10000) {
        this.actList = qosToAdd.result.actionOptions;
        this.dscpList = qosToAdd.result.dscpOptions;
        this.sgList = qosToAdd.result.streamGroupOptions;
      }
      if (qosData.code === 10000) {
        this.ruleForm.description = qosData.result.description;
        this.ruleForm.name = qosData.result.name;
        this.ruleForm.id = qosData.result.id;
        this.tabIndex = qosData.result.qosPolicyList.length;
        this.editableTabsValue = "1";
        this.ruleForm.editableTabs = qosData.result.qosPolicyList.map((item, index) => {
          return {
  ...
  created() {
    ...
    //   if (res.code === 200) {
    //     this.actList = res.result.actionOptions;
    //     this.dscpList = res.result.dscpOptions;
    //     this.sgList = res.result.streamGroupOptions;
    //   }
    // });
  },
  mounted() {
