...
export default {
  ...
  methods: {
    ...
    // Item deletion
    remove(row) {
      var show83 = localStorage.getItem("show83");
      if(show83 == 'true'){
        ...
        this.$confirm(this.$t("vsr1000.qosremind[1]")+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue83\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", {
        ...
        .then(() => {
          bgpCidrDel({ ids: [row.id] }).then((res) => {
            if (res.code === 10000) {
              ...
              this.page = 1;
              this.dataInit();
              localStorage.setItem("show83",localStorage.getItem("checkedValue83"));
            } else if (res.code !== 10000) {
              this.page = 1;
              this.dataInit();
              this.$message({
    ...
    removeList() {
      if (this.multipleSelection.length) {
        ...
        var show84 = localStorage.getItem("show84");
        if(show84 == 'true'){
          ...
          this.$confirm(this.$t("vsr1000.qosremind[1]")+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue84\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", {
          confirmButtonText: this.$t("vsr1000.OK"),
          cancelButtonText: this.$t("vsr1000.cancel"),
          dangerouslyUseHTMLString: true,
          type: "warning",
          ...
          .then(() => {
            bgpCidrDel({ ids }).then((res) => {
              if (res.code === 10000) {
                ...
                this.page = 1;
                this.dataInit();
                localStorage.setItem("show84",localStorage.getItem("checkedValue84"));
              } else if (res.code !== 10000) {
                this.page = 1;
                this.dataInit();
                this.$message({
