<template>
  <div class="contents">
    <div :class="`inner devList ${lang == 'en' ? 'innerEn' : ''}`">
      <div class="top">
        <h2>{{ $t("Mobile.addTrustPolicy") }}</h2>
        <i @click="$parent.showTrust = false">
      ...
      <div class="footer">
        ...
        <button class="cancel" @click="$parent.showTrust = false">
          {{ $t("Mobile.cancel") }}
        </button>
      </div>
    </div>
...
export default {
  ...
  methods: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let params = {
            ...
            clientname:this.source2[0].name,
          };

          addTrustPolocy(params).then((res) => {
            if (res.code === 10000) {
              this.$refs.form.resetFields();
              this.$parent.showTrust = false;
              this.$message({
    ...
    dataInit(type,clientId,clientName,tenantId) {
      this.isSave = false;
      ...
      this.source2[0].id = clientId;
      this.source2[0].name = sName;
      this.form.sourceName = clientId;

      trustSelectOptions({tenantId:tenantId}).then((res)=>{
        if (res.code === 10000) {
