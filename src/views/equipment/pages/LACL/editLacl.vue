<template>
    <div class="editLacl">
        <div class="content">
            <div class="top">
                LAN-ACL {{$t('vsr100.acl.edit')}}
                <span @click="close"></span>
            </div>
            <div class="form">
                <el-form  :model="form" :rules="rules" ref="form" label-width="150px" class="demo-ruleForm" @submit.native.prevent>
                  ...
                  <el-form-item :label="$t('vsr100.acl.sourceIP')" prop="Ip">
                      <el-input v-model="form.Ip" autocomplete="off" size="small"></el-input>
                  </el-form-item>
                  ...
                  <el-form-item :label="$t('vsr100.acl.goalPort')" v-show="codeShow" prop="goalCode">
                      <el-input v-model="form.goalCode" size="small"></el-input>
                  </el-form-item>
                  <div class="control">
    ...
    methods: {
      ...
      save(formName){
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if(this.form.num === this.form.confirmNum){//编辑序列号没有改变
                                                       // Edit sequence number has not changed
              this.edit(params)
            }else{
              findLanAclOderNum({orderNum:this.form.num}).then(res=>{
                  if(res.result==true){
                    this.$message({
                      message: '编辑失败，该序列号已经使用',
                      type: 'warning'
                    });             
