  ...
  <div class="addStatic">
    <div class="context">
      ...
      <div class="item" :style="lang=='en'?'':'marginLeft:70px'">
        ...
        >
          <el-form-item :label="$t('vsr100.static.IPSite')" prop="ip">
            <div class="item-ipt">
              <el-input
                v-model="form.ip"
                @blur="zhuan"
                :placeholder="$t('vsr100.static.enter')"
                size="small"
              ></el-input>
            </div>
          </el-form-item>
          ...
          <el-form-item :label="$t('vsr100.static.tag')" prop="title">
            <div class="item-ipt">
              <el-input v-model.number="form.title" placeholder="1-65535" size="small"></el-input>
            </div>
          </el-form-item>
...
export default {
      ...
      if (value) {
        ...
        if (reg.test(val)) {
          ...
          callback();
            ...
            callback(new Error("值的范围为0-65535"));
          }
        } else {
  ...
  methods: {
    ...
    save(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          sceneNodeAddStatic(params).then(res => {
            // console.log(res)
            if (res.code == 10000) {
              this.$message({
  ...
  created() {
    ...
    // })
    getFindStaticOutInter({ devId: sessionStorage.getItem("edgeDevId") }).then(
      res => {
        if (res.code === 10000) {
          this.apiArr = res.result;
        }
