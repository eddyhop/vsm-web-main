<template>
  <div class="editStatic">
    <div class="context">
      <div class="title">
        {{$t('subscription.dhcpServer.editDHCPServer')}}
      ...
      <div class="item" :style="lang == 'en' ? '' : 'marginLeft:70px'">
        ...
        >
          ...
          >
            <div class="item-ipt">
              <el-input v-model="form.leaseTime" size="small"></el-input>
            </div>
          </el-form-item>
          <el-form-item
          ...
          <div class="contr">
            <button type="primary" v-debounce="save" class="save">
              {{ $t("subscription.save") }}
            </button>
...
<script>
import {dhcpGetServiceByDeviceId, getLogicLanPort, dhcpAddOrEditService, dhcpCheckIp} from "@/services";
export default {
  props: {
    dhcpId: {
      type: String
    ...
    return {
      ...
      rules: {
        ...
        dnsIp: [
          {
            pattern: /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/,
            message: this.$t("vsr100.dhcp.remind[9]"),trigger: "blur"
          }
        ],
        leaseTime: [
          {
  ...
  methods: {
    close() {
      this.$parent.editDHCPServerShow = false;
    ...
    getDHCP(){
      dhcpGetServiceByDeviceId({ deviceId: this.deviceId }).then(res => {
        if (res.code === 10000) {
          this.form.name = res.result.name;
          this.form.description = res.result.description;
          this.form.ipStart = res.result.ipStart;
          this.form.ipEnd = res.result.ipEnd;
          this.form.leaseTime = res.result.leaseTime;
          this.form.dnsIp = res.result.dnsIp;
          this.form.id = res.result.id;
          this.form.logID = res.result.logID;
        }
      });
