...
<script>
import { editStaticDhcp } from "@/services";
export default {
  props: {},
  components: {},
  data() {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      let data = {
        dhcpId: id,
        hostName: this.ruleForm.name,
        id: this.ruleForm.id,
        ip: this.ruleForm.ip,
        macAddr: this.ruleForm.main,
        status: 0
      };

      this.$refs[formName].validate(valid => {
        if (valid) {
          editStaticDhcp(data).then(res => {
            if (res.code === 200) {
              this.$message({
                type: "success",
                message: "编辑成功!"
    ...
    //获取当前编辑数据
    // Gets the current edit data
    dataInit(row) {
      console.log(123123123123);
      this.ruleForm.name = row.hostName;
      this.ruleForm.ip = row.ip;
      this.ruleForm.id = row.id;
      this.ruleForm.main = row.macAddr;
    }
  },
  created() {},
  mounted() {}
