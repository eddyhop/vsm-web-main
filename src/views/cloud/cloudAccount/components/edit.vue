<template>
  <div class="accountAdd" id="accountEdit">
    <div class="inner">
      <div class="top">
        <h2>{{$t('cloudAccount.editTitle')}}</h2>
        <i @click="close">
        ...
        >
          ...
          <el-form-item :label="$t('cloudDev.submit')" prop="radio">
            <!-- <el-select filterable
              ...
              :placeholder="$t('cloudDev.remind[0]')"
              @change="changeCloudServer(ruleForm.manufacturerName)"
            >
              <el-option
                v-for="item in Types"
                :key="item.value"
            ...
            <el-radio-group v-model="ruleForm.radio" class="radioList" disabled>
              <div class="radioItem">
          ...
          <div v-if="ruleForm.radio=='AWS_CHINA' || ruleForm.radio=='AWS'">
            ...
            <el-form-item label="Access Key" prop="AWSCaccess" v-if="ruleForm.radio=='AWS_CHINA'">
              <el-input type="password" v-model="ruleForm.AWSCaccess" :placeholder="$t('cloudAccount.remind[1]')">
              </el-input>
            </el-form-item>
            <el-form-item label="Access Key" prop="AWSaccess" v-if="ruleForm.radio=='AWS'">
              <el-input type="password" v-model="ruleForm.AWSaccess" :placeholder="$t('cloudAccount.remind[1]')">
              </el-input>
            </el-form-item>
            <el-form-item label="Secret Key" prop="AWSCsecret" v-if="ruleForm.radio=='AWS_CHINA'">
              <el-input type="password" v-model="ruleForm.AWSCsecret" :placeholder="$t('cloudAccount.remind[1]')">
              </el-input>
            </el-form-item>
            <el-form-item label="Secret Key" prop="AWSsecret" v-if="ruleForm.radio=='AWS'">
              <el-input type="password" v-model="ruleForm.AWSsecret" :placeholder="$t('cloudAccount.remind[1]')">
              </el-input>
            </el-form-item>
          </div>
          <div v-if="ruleForm.radio=='ALIYUN'">
            <el-form-item label="Access Key" prop="ALaccess">
              <el-input type="password" v-model="ruleForm.ALaccess" :placeholder="$t('cloudAccount.remind[1]')"></el-input>
            </el-form-item>
            <el-form-item label="Secret Key" prop="ALsecret">
              <el-input type="password" v-model="ruleForm.ALsecret" :placeholder="$t('cloudAccount.remind[1]')"></el-input>
            </el-form-item>
          </div>
...
export default {
  props: ["scopeEdit"],
  data() {
    icon:"el-input__icon el-icon-view";passw:"password";
    const CloudNameRule = (rule, value, callback) => {
      var regEn = /[`!@#$%^&*()_+<>?:"{},.\/;'[\]]/im,
        regCn = /[·！#￥（——）：；“”‘、，|《。》？、【】[\]]/im;
      if (regEn.test(value) || regCn.test(value)) {
        return callback(new Error("您输入了非法字符，请重新输入"));
      } else {
        callback();
      }
    };
...
　　　　　　　　　　//点击图标是密码隐藏或显示
    ...
    // Click Edit to assign values to all input fields in the edit box
    editAssignment(scopeEdit) {
      console.log(scopeEdit);
      // this.ruleForm.manufacturerName = scopeEdit.manufacturerName;
      this.ruleForm.name = scopeEdit.accountName;
      this.ruleForm.id = scopeEdit.id;
      this.ruleForm.radio = scopeEdit.type;
      switch (scopeEdit.type) {
        case "ALIYUN":
          this.ruleForm.ALaccess = scopeEdit.accessKey;
          this.ruleForm.ALsecret = scopeEdit.accessSecret;
          break;
        case "TENCENT":
          this.ruleForm.TXaccess = scopeEdit.accessKey;
          this.ruleForm.TXsecret = scopeEdit.accessSecret;
          break;
        case "AWS_CHINA":
          this.ruleForm.AWSCaccess = scopeEdit.accessKey;
          this.ruleForm.AWSCsecret = scopeEdit.accessSecret;
          this.ruleForm.awsCId = JSON.parse(scopeEdit.addition).awsId;
          break;
        case "AWS":
          this.ruleForm.AWSaccess = scopeEdit.accessKey;
          this.ruleForm.AWSsecret = scopeEdit.accessSecret;
          this.ruleForm.awsId = JSON.parse(scopeEdit.addition).awsId;
          break;
        case "AZURE":
          this.ruleForm.azureaccessKey = scopeEdit.accessKey;
          this.ruleForm.azureaccessSecret = scopeEdit.accessSecret;
          this.ruleForm.azureDirectoryId = JSON.parse(
            scopeEdit.addition
          ).azureDirectoryId;
          this.ruleForm.azureSubId = JSON.parse(scopeEdit.addition).azureSubId;
          break;
        case "AZURE_CHINA":
          this.ruleForm.azureCaccessKey = scopeEdit.accessKey;
          this.ruleForm.azureCaccessSecret = scopeEdit.accessSecret;
          this.ruleForm.azureCDirectoryId = JSON.parse(
            scopeEdit.addition
          ).azureDirectoryId;
          this.ruleForm.azureCSubId = JSON.parse(scopeEdit.addition).azureSubId;
          break;
        default:
          break;
      }
