            ...
            <div class="form">
                <el-form :model="form" :rules="rules" ref="form" label-width="150px" :inline="true" class="demo-ruleForm" @submit.native.prevent>
                  <el-form-item :label="$t('vsr100.acl.index')" prop="num">
                      <!-- <el-input v-model.number="form.num" autocomplete="off" size="small"></el-input> -->
                      {{form.num}}
                  </el-form-item>
                  <el-form-item :label="$t('vsr100.acl.protocol')" prop="value">
                     <div class="a">
                        <el-select filterable v-model="form.value" :placeholder="$t('vsr100.acl.choose')" @change="chose(form.value)">
                          <el-option
                  ...
                  <el-form-item :label="$t('vsr100.acl.sourcePort')" prop="code" v-show="codeShow">
                      <el-input v-model="form.code" placeholder="1-65535"></el-input>
                  </el-form-item>
                  <el-form-item :label="$t('vsr100.acl.destIP')" prop="goalIp">
                      <el-input v-model="form.goalIp"></el-input>
                  </el-form-item>
                  <el-form-item :label="$t('vsr100.acl.goalPort')" prop="goalCode" v-show="codeShow">
                      <el-input v-model="form.goalCode" placeholder="1-65535"></el-input>
                  </el-form-item>
                  <div class="control">
...
<script>
import {setVpnAcl} from '@/services'
export default {
    data() {
      return {
        codeShow:false,
    ...
    methods: {
      ...
      close(){
        this.$parent.editVShow = false
        this.$refs.form.resetFields();
      },
      save(formName){
        let params = {
          destinationIp:this.form.goalIp,//目的ip
                                         // Destination IP
          destinationIpPort:this.form.goalCode,//目的端口
                                               // Destination port
          id:this.form.id,//id
          deviceId:this.$parent.mid,//mid
          protocol:this.form.value,//协议
                                   // agreement
          randomId:parseInt((Math.random() + 1) * Math.pow(10, 6 - 1))+'',//随机Id
                                                                          // Random Id
          sourceIp:this.form.Ip,//源ipV
                                // Source ipV
          sourceIpPort:this.form.code,//源端口
                                      // Source port
          sortOrder:Number(this.form.num)
        }
         this.$refs[formName].validate((valid) => {
          if (valid) {
            setVpnAcl(params).then(res=>{
              if(res.code==10000){
                this.$message({
