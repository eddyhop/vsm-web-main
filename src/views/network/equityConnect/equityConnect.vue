<template>
  ...
  <div class="equityConnect">
    ...
    >
      <div class="netWorkItem" v-for="(item) in netTableList" :key="item.sceneId">
        <div class="title">
          ...
          <div class="submit">
            <span>{{item.equalType===1 ? 'Site-to-Site' : item.equalType===2 ? 'Site-to-Cloud' : 'VPC-to-VPC'}}</span>
        ...
        <div class="Transit-tableList">
          ...
          >
            ...
            <el-table-column prop="deviceName" :label="$t('DeviceMan.operation')" align="left" :resizable="false">
              <template slot-scope="scope">
                <el-button
                  type="text"
                  size="small"
    ...
    <addEquityConnect ref="addEquityConnect" v-show="isShow" />
    <CidrSet ref="CidrSet" v-show="CidrShow" />
    <netWorklink ref="netWorklink" v-show="netWorkShow" />
  </div>
</template>
<script>
import addEquityConnect from "./components/addEquityConnect";
...
export default {
  ...
  methods: {
    ...
    changeVrfName(val) {
      sceneNodeGetNetList({ vrfId: val, type: 2 }).then((res) => {
        if (res.code === 10000) {
    ...
    getSelectList() {
      sceneNodeGetTenantVrfList().then((res) => {
        if (res.code === 10000) {
          ...
          this.dataInit(params);
          sceneNodeGetNetList({ vrfId: this.vrfName, type: 2 }).then((res) => {
            if (res.code === 10000) {
              this.netList = res.result;
            }
          });
    ...
    addLink() {
      this.isShow = true;
      this.$refs.addEquityConnect.dataInit();
    },
    // //cidr
    // cidrSet(row) {
    //   this.CidrShow = true;
    ...
    // Delete the scene
    removeScene(row) {
      var show76 = localStorage.getItem("show76");
      if(show76 == 'true'){
          sceneDelete({ sceneId: row.sceneId, sceneType: 2 }).then((res) => {
            if (res.code === 10000) {
              this.$message({
                type: "success",
        ...
        this.$confirm(this.$t("vsr1000.qosremind[1]")+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue76\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", {
        ...
        .then(() => {
          sceneDelete({ sceneId: row.sceneId, sceneType: 2 }).then((res) => {
            if (res.code === 10000) {
              ...
              localStorage.setItem("show76",localStorage.getItem("checkedValue76"));
            } else if (res.code !== 10000) {
              this.page = 1;
              this.dataInit(
                {
