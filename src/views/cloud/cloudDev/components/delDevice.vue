<template>
  <div class="closeAws">
    <div class="content">
      <p>{{this.$t('cloudDev.remind[6]')}}{{delMsg.instanceName}}{{this.$t('cloudDev.second')}}？</p>
      <div class="submit">
        <button
          class="save"
          type="primary"
...
<script>
import { delDevice } from "@/services"
export default {
  props: ["delMsg"],
  components: {},
  data() {
  ...
  methods: {
    submitForm(){
      delDevice({
        id:this.delMsg.id,
        cloudAccountId:this.delMsg.cloudAccountId,
        regionId:this.delMsg.regionId
      }).then(res => {
        if(res.code === 10000){
          if(this.$parent.tableData.length == 1 && this.$parent.page != 1){
            this.$parent.page = this.$parent.page - 1;
          }
          this.$message.success(this.$t('cloudDev.remind[5]'));
