...
export default {
  ...
  methods: {
    ...
    // Item deletion
    remove(row) {
      var show77 = localStorage.getItem("show77");
      if(show77 == 'true'){
        ...
        this.$confirm(this.$t("vsr1000.qosremind[1]")+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue77\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", {
        ...
        .then(() => {
          bgpCidrDel({ ids: [row.id] }).then((res) => {
            if (res.code === 10000) {
              ...
              this.page = 1;
              this.dataInit();
              localStorage.setItem("show77",localStorage.getItem("checkedValue77"));
            } else if (res.code !== 10000) {
              this.page = 1;
              this.dataInit();
              this.$message({
    ...
    removeList() {
      if (this.multipleSelection.length) {
        ...
        var show78 = localStorage.getItem("show78");
        if(show78 == 'true'){
          ...
          this.$confirm(this.$t("vsr1000.qosremind[1]")+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue78\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", {
          confirmButtonText: this.$t("vsr1000.OK"),
          cancelButtonText: this.$t("vsr1000.cancel"),
          dangerouslyUseHTMLString: true,
          type: "warning",
          ...
          .then(() => {
            bgpCidrDel({ ids }).then((res) => {
              if (res.code === 10000) {
                ...
                this.page = 1;
                this.dataInit();
                localStorage.setItem("show78",localStorage.getItem("checkedValue78"));
              } else if (res.code !== 10000) {
                this.page = 1;
                this.dataInit();
                this.$message({
