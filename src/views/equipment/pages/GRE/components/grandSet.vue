<template>
  <div class="XLANgrande">
    <div class="context">
      <h4 class="title">
        <span>GRE-{{$t('vsr1000.expertSet')}}</span>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        mss: [
          { required: true, message: this.$t("vsr1000.remind2[4]"), trigger: "blur" },
  ...
  methods: {
    ...
    async getSelectList(row) {
      ...
      // let Setting = await getGreSuper(params2);
      // if (Setting.result) {
      //   let data = {
