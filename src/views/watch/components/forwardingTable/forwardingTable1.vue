<template>
  <div class="logical">
    <!-- 逻辑端口 -->
    <div class="operate">
      <button @click="qhuan(1)" class="flBut" :class="{'onBut':qhuanInt==1}" v-control>{{$t('Monit.ipv4')}}</button>
      <button @click="qhuan(2)" class="flBut" :class="{'onBut':qhuanInt==2}" v-control>{{$t('Monit.arpTable')}}</button>
    ...
    <div v-if="qhuanInt==1" class="tableView">
      <el-table
        ...
        size="small"
      >
        <el-table-column prop="destinationIPOrMask" :label="$t('Monit.destinationIPOrMask')" show-overflow-tooltip :resizable="false"></el-table-column>
        <el-table-column prop="nextHop" :label="$t('Monit.nextHop')" show-overflow-tooltip :resizable="false"></el-table-column>
        <el-table-column prop="interfaceName" :label="$t('Monit.interfaceName')" show-overflow-tooltip :resizable="false"></el-table-column>
      </el-table>
      <div class="page">
        <el-pagination
          :page-sizes="[5, 10]"
    ...
    <div v-if="qhuanInt==2" class="tableView">
      <el-table
        ...
        size="small"
      >
        <el-table-column prop="ip" :label="$t('Monit.ip')" show-overflow-tooltip :resizable="false"></el-table-column>
        <el-table-column prop="macAddress" :label="$t('Monit.macAddress')" show-overflow-tooltip :resizable="false"></el-table-column>
        <el-table-column prop="interfaceName" :label="$t('Monit.interfaceName')" show-overflow-tooltip :resizable="false"></el-table-column>
        <el-table-column prop="type" :label="$t('Monit.type')" show-overflow-tooltip :resizable="false"></el-table-column>
      </el-table>
      <div class="page">
        <el-pagination
          :page-sizes="[5, 10]"
  ...
  methods: {
    ...
    choseDel() {
      ...
      if (this.tableData.length == 0) {
        ...
        var show105 = localStorage.getItem("show105");
        if(show105 == 'true'){
          
            logicalPortDel({logicalPortIds:ids.join(','),deviceSerialNumber:this.deviceSerialNumber,tenantId:this.tenantId}).then(res => {
              if (res.code == 10000) {
                this.$message({
                  type: "success",
          ...
          this.$confirm(this.$t('vsr1000.remind[1]')+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue105\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", {
          ...
          .then(() => {
            logicalPortDel({logicalPortIds:ids.join(','),deviceSerialNumber:this.deviceSerialNumber,tenantId:this.tenantId}).then(res => {
              if (res.code == 10000) {
                ...
                this.getList();
                localStorage.setItem("show105",localStorage.getItem("checkedValue105"));
              } else{
                let strArr = res.message.split(',')
                let str = "";
                strArr.forEach(item => {
    ...
    del(row) {
      var show106 = localStorage.getItem("show106");
      if(show106 == 'true'){
        
          let params = {
            logicalPortIds:row.id,deviceSerialNumber:this.deviceSerialNumber,tenantId:this.tenantId
          }
        ...
        this.$confirm(this.$t('vsr1000.remind[1]')+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue106\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", {
          confirmButtonText: this.$t('vsr1000.OK'),
          cancelButtonText: this.$t('vsr1000.cancel'),
          dangerouslyUseHTMLString: true,
          type: "warning"
        ...
        .then(() => {
          ...
          logicalPortDel(params).then(res => {
            if (res.code == 10000) {
              ...
              this.getList();
              localStorage.setItem("show106",localStorage.getItem("checkedValue106"));
            } 
            else{
              this.$message({
                type: "warning",
    ...
    handleClickSet(row) {
      ...
      async function getAll() {
        ...
        let search = logicalPortExtraGet({logicalPortId:row.id}).then(res=>{
