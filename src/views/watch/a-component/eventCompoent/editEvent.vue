<template>
  <div class="addzh" id="adEvent">
    <div class="cont">
      <div class="title">
        {{$t('Alarm.edit')}} {{$t('Alarm.eventDefinition')}}
      ...
      <div class="even">
        <el-form :model="inputMessage" :rules="rules" ref="form" label-width="100px" class="demo-ruleForm"  @submit.native.prevent>
          ...
          <span class="ecee">
            ...
            >
              <div class="region" style="display:flex;margin-left:35px">
                ...
                <el-select filterable v-model.number="inputMessage.eventTypeName" @change="choseEve" :disabled="couldChose">
          ...
          <!-- <el-form-item
            :label="$t('Alarm.eventThresholdValue')"
            prop="thresholdNum"
            :rules="[{ required: true, message: $t('Alarm.hint3')},
            { pattern: /^([1-9]\d*|[0]{1,1})$/, message: $t('Alarm.hint13')}]"
          >
            <div class="threshold">
              <el-input
                v-model="inputMessage.thresholdNum"
                autocomplete="off"
                :placeholder="$t('Alarm.value')"
              ></el-input>
              <span v-if="inputMessage.eventTypeName===$t('Alarm.flow')">bps</span>
              <span v-else>%</span>
            </div>
          </el-form-item> -->
          <el-form-item
...
export default {
  data() {
    ...
    var name = (rule, value, callback)=>{
        if(value){
          checkWarningEventName({eventName:value,tenantId:this.tenantId,id:sessionStorage.getItem('eventId')}).then(res=>{
            if(res.code==10000){
              callback()
            }else{
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.choseList.map(item=>{
        if(item.eventNum==this.inputMessage.eventTypeName){
