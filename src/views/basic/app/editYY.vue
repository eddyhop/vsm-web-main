<template>
    <div class="edityy">
        <div class="cont">
            <div class="title">
                 {{$t('App.edit')}} {{$t('App.applicationGroups')}}
            ...
            <div class="even">
                <el-form :model="form" :rules="rules" ref="form" label-width="150px" @submit.native.prevent>
                    ...
                    <el-form-item>
                        <!-- <span class="remaind">{{$t('yinyong.world')}}</span> -->
                        <div class="contr">
                            <button v-debounce="save">{{$t('App.save')}}</button>
        ...
        var checkName = (rule, value, callback) => {
            if (value === '') {
                ...
                if(value.length>32){
                    ...
                    appNamExit({verifyName:value,verifyScope:tenantId,verifyId:this.form.id}).then(res=>{
                        if(this.flag){
        ...
        chosePort(val){
            for(let i in this.form.options2){
                if(val==this.form.options2[i].id){
                    if(this.form.options2[i].name=='TCP'||this.form.options2[i].name=='UDP'){
                        this.log=false
