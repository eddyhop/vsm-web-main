<template>
  <div class="addSTATIC">
    <div :class="`context ${lang=='en'?'contextEn':''}`">
      <h4 class="title">
        <span>{{$t('vsr1000.add')}} STATIC-IPV{{type === 0 ? '4' : '6'}}</span>
      ...
      <div class="event">
        ...
        >
          <el-form-item :label="$t('vsr1000.tenantRoutingDomain')" prop="vrfName">
            <!-- <el-select filterable v-model="ruleForm.vrfName" :placeholder="$t('vsr1000.choose')">
              <el-option
                v-for="item in ruleForm.vrfNameArr"
                :key="item.id"
                :label="item.name"
                :value="item.id"
              ></el-option>
          ...
          <el-form-item :label="$t('vsr1000.targetSite')" prop="ip">
            <el-input v-model="ruleForm.ip" @blur="zhuan"></el-input>
          </el-form-item>
          <el-form-item :label="$t('vsr1000.distance')" prop="dis">
            <el-input v-model="ruleForm.dis" placeholder="0"></el-input>
          </el-form-item>
          <el-form-item :label="$t('vsr1000.nextHop')" prop="nextHopIp">
            <el-input v-model="ruleForm.nextHopIp"></el-input>
          </el-form-item>
          <el-form-item :label="$t('vsr1000.outInterface')" prop="outInterface">
            ...
            >
              ...
              >
                ...
                <el-option
                  v-for="item in ruleForm.outNameArr"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                ></el-option>
...
export default {
  ...
  data() {
    ...
    //           callback();
    //         } else {
    //           callback(new Error("您输入的IP地址已存在"));
    // Callback (new Error(" The IP address you typed already exists ");
    //         }
    //       });
    //     } else {
    //       callback();
    ...
    let ipRepeatRule = (rule, value, callback) => {
      if (value) {
        if (this.ruleForm.vrfName.length && this.ruleForm.ip) {
          checkRepeatIp({
    ...
    let ipRuleChange = (rule, value, callback) => {
      if (value) {
        if (this.ruleForm.vrfName.length) {
          callback();
          // if (value == "0.0.0.0/0") {
    ...
    // let nextHoopChange = (rule, value, callback) => {
    //   if (value) {
    //     checkRepeatNextIp({
    //       deviceId: sessionStorage.getItem("deviceId"),
    //       ip: value,
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      let data = {
        ...
        title: "",
        type: this.type,
        loginPortId: this.ruleForm.outName,
        vrfId: this.ruleForm.vrfName,
