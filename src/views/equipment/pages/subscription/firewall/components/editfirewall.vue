<template>
  <div class="editStatic">
    <div class="context">
      <div class="title">
        {{$t('subscription.firewall.editfirewall')}}
      ...
      <div class="item" :style="lang == 'en' ? '' : 'marginLeft:70px'">
        ...
        >
          ...
          <el-form-item :label="$t('subscription.firewall.applicationGroup')" prop="appGroup">
            <div class="item-ipt">
              <el-select filterable clearable v-model="form.appGroup"  :disabled="appGD"  :placeholder="$t('subscription.firewall.applicationGroup')" size="small">
                <el-option
                    v-for="item in appG"
                    :key="item.id"
                    :label="item.name"
          ...
          <el-form-item :label="$t('subscription.firewall.application')" prop="application">
            <div class="item-ipt">
              <el-select filterable clearable v-model="form.application"  :disabled="appD"    :placeholder="$t('subscription.firewall.application')" size="small">
                <el-option
                    v-for="item in app"
                    :key="item.id"
                    :label="item.name"
          ...
          <el-form-item :label="$t('subscription.firewall.prioruty')" prop="priority">
            <div class="item-ipt">
              <el-input v-model.number="form.priority" size="small"></el-input>
            </div>
          </el-form-item>
          <div class="contr">
...
<script>
import {firewalldAddOrEdit, firewalldIsOnlyApp, firewalldIsOnlyName, firewalldIsOnlyPriority, GetFirewalldSel} from "@/services";
export default {
  data() {
    ...
    let checkApp = (rule,value,callback) => {
      if(value){
        if(this.form.application!=this.oldapp) {
    ...
    let checkAppG = (rule,value,callback) => {
      if(value){
        if(this.form.appGroup!=this.oldappG) {
  ...
  methods: {
    close() {
      this.$parent.editfirewallShow = false;
    ...
    intiDate(row){
      ...
      GetFirewalldSel({deviceId:this.deviceId,tenantId:this.tenantId}).then(res=>{
        ...
        if(row.appGroup){
          ...
          this.oldappG  = row.appGroup;
        }

        if(row.app){
          this.form.application = row.app;
          this.oldapp =  row.app;
        }
        ...
        if( this.form.application!="" ){
          ...
          let b = true;
          this.app.find((item)=> {
            if(item.id == this.form.application){
              b=false;
            }
          });
          if(b){
      ...
      this.form.interface = row.interfaceID;
