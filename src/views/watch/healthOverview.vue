    ...
    <div class="top">
      <div class="search">
       ...
       <!--
        ...
        -->
        <h3>
          {{$t('vsnOperaions.vsnOperaions')}}
          <img src="@/assets/icon_right@2x.png" alt />
          <span>{{$t('vsnOperaions.diagnosticsAndRepair')}}</span>
        </h3>
      </div>
      <el-row>
...
import {getDevSelect, getDevice, userInfo} from "@/services";
...
export default {
  ...
  data() {
    return {
      ...
      tabsData: [{
          ...
          //VPN状态
        name: this.$t("Diag.naasTunnelsStatus"),
        id: 2,
        componentNmae: vpnStatus,
      },{
          //路由表
        name: this.$t("Monit.routingTable"),
        id: 3,
  ...
  methods: {
    ...
    async  tabListInit(data) {
      ...
      userInfo().then(res => {
        ...
        if(userData.username == "vsmAdmin" && userData.tenantName == "default"){
          ...
          vsr1100Naas = [0, 1, 14, 16, 17, 18, 19, 20, 21, 31, 32, 33, 34, 36, 38, 39];
          ...
          vsr1100Naas = [0, 1, 14,16, 32, 33, 34, 36];
    ...
    edgeChange(){
      ...
      if(!isEmpty(this.edgeName)) this.getDeviceinfo(this.edgeName );
    }
  },
   created() {
     this.edgeListInit();
     ...
     if(!isEmpty(this.$route.query.devData)) {
