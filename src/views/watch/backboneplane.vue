    ...
    <div class="top">
      <div class="search">
        ...
        <h3>
          ...
          <span>{{ $t("naas.backboneControlPlane") }}</span>
        </h3>
      </div>
      <el-row>
...
<script>
import naasHealthOverview from "@/views/watch/components/naasHealthOverview/naasHealthOverview.vue";
import ospfHealthOverview from "@/views/watch/components/ospfHealthOverview/ospfHealthOverview.vue";
import bfdHealthOverview from "@/views/watch/components/bfdHealthOverview/bfdHealthOverview.vue";
import {
  getNaaSDevSelect,
  getDevice,
  getDevSelect,
...
export default {
  ...
  data() {
    return {
      ...
      tabsData: [
          {
            //健康概览
            name: this.$t("Monit.bgpStatus"),
            id: 0,
            componentNmae: naasHealthOverview,
          },{
            //接口状态
            name: this.$t("Monit.ospfStatus"),
            id: 1,
            componentNmae: ospfHealthOverview,
          },{
            //VPN状态
          name: this.$t("Monit.bfdStatus"),
          id: 2,
          componentNmae: bfdHealthOverview,
        }
      ],
      tabList: [
        {
  ...
  methods: {
    ...
    async tabListInit(data) {
      ...
      userInfo().then((res) => {
        userData = res.result;
        if (
          userData.username == "vsmAdmin" &&
          userData.tenantName == "default"
        ) {
          vsr1100NaaS = [0, 1, 2];
        } else {
          vsr1100NaaS = [0, 1, 2];
      ...
      // 注释时间 11-19
      vsr1100NaaS.forEach((item) => {
        this.tabsData.push(this.tabList.find((el) => el.id == item));
      });
    },
    async edgeListInit() {
      // let tenantId = JSON.parse(localStorage.getItem("userData")).type==1? localStorage.getItem("tenantValue") ? localStorage.getItem("tenantValue") : '': JSON.parse(localStorage.getItem("userData")).tenantId;
      let tenantId = sessionStorage.getItem("tenantValue");
      //先用所有设备进行测试 getnaas之后取消注释
      //getNaaSDevSelect({ tenantId: tenantId }).then((res) => {
      getNaaSDevSelect({ tenantId: tenantId }).then((res) => {
    ...
    getDeviceinfo(deviceId) {
      getDevice({ deviceId: deviceId }).then((data) => {
        ...
        let params = {
          ...
          location: res.deviceLocation,
        };
        this.$router.push({
          path: "/naas/backboneplane",
  ...
  created() {
    ...
    /*
     ...
     if(this.devData.deviceId){
       this.edgeName = this.devData.deviceId;
       this.getDeviceinfo(this.edgeName );
     } */
  },
  watch: {},
