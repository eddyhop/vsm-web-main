  ...
  <div class="chartsMain">
    <div class="time">
      <el-tabs v-model="activeName" @tab-click="handleClick" style="padding-right: 20px">
        <el-tab-pane :label="$t('Home.lastTenMinutes')" name="first"></el-tab-pane>
        <el-tab-pane :label="$t('Home.today')" name="second"></el-tab-pane>
        <el-tab-pane :label="$t('Home.week')" name="third"></el-tab-pane>
        <el-tab-pane :label="$t('Home.month')" name="fourth"></el-tab-pane>
      </el-tabs>
...
import {
  ...
  devicePortNetworkFlowChart,
  onlineStateChart, userInfo
} from "@/services";
  ...
  methods: {
    chart0() {
      ...
      let dataChange = () => {
        ...
        this.onlineData.time.forEach((item, index) => {
          arr.push({
            name: this.getTime(Number(item)),
            value: [
              this.getTime(Number(item)),
              this.onlineData.data[0].data[index],
            ],
          });
        });
        return arr;
      ...
      this.myChart0 = this.$echarts.init(document.getElementById("chart0"));
      if (this.onlineData.time.length) {
        dataArr = dataChange();
      } else {
        //模拟数据
        // Simulated data
    ...
    choseLastTenMinutes() {
      this.tabIndex = 1;
      this.startDate = this.getTime(new Date() - (10 * 60000));
      ...
      this.$emit('start-date-change', this.startDate);
    ...
    async dataInit(params) {
     ...
     var utc = parseInt(utcDate);
      ...
      //------------------------------------------------------------------------
      let onlineData = await onlineStateChart(params);
      //cpu图
      // CPU figure
     ...
     // let PortNetworkFlowData = await devicePortNetworkFlowChart(params);
      this.onlineData = onlineData.result;
    ...
    chartsInit() {
      var _this = this;
      _this.chart0();
      //console.log("chart000000000000000000000000000000000000");
      _this.chart1();
     // console.log("chart111111111111111111111111111111111111");
      _this.chart2();
      //console.log("chart222222222222222222222222222222222222");
      _this.chart3();
     // console.log("chart333333333333333333333333333333333333");
      // this.chart4();
      setTimeout(() => {
        _this.myChart1.resize();
        _this.myChart2.resize();
