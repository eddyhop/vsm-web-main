<template>
  <div class="addStatic">
    <div class="context">
      <div class="title">
        {{$t('subscription.vpns.addSiteVPN')}}
      ...
      <div class="item" :style="lang=='en'?'':'marginLeft:70px'">
        ...
        >
          ...
          <el-form-item :label="$t('subscription.vpns.innerIPAddress')" prop="innerIp">
            <div class="item-ipt">
              <el-input v-model="form.innerIp" size="small"  :placeholder="$t('subscription.vpns.innerIPAddress')" ></el-input>
              <span class="lxSpan">/30</span>
            </div>
          </el-form-item>
          <div class="contr">
...
export default {
  ...
  methods: {
    close() {
      this.$parent.addsiteVPNsShow = false;
    ...
    getdic(){
      getVpnSelect({deviceId:this.deviceId,tenantId:this.tenantId,userType: this.userType}).then(res=>{
        ...
        this.tunnel.find((item)=> {
          if(item.name == "GRE-over-IPsec"){
            this.form.tunnelEnacaplation = item.id;
          }
        });
      })
    },
