          ...
          <div class="item" >
            <el-form-item :label="$t('vsr1000.physicalInterface')" prop="portId"  >
              <el-select filterable multiple v-model="form.portId" :placeholder="$t('vsr1000.remind[28]')" @change="changePort(form.portId)">
                <el-option v-for="item in form.physicalInterfaceList" :key="item.id" :label="item.name" :value="item.name"></el-option>
              </el-select>
            </el-form-item>
          </div>
...
<script>
import { logicalPortAdd , isMaskRepeat ,isIPVSRepeat ,logicalPortName,vlanIdAndPortIdExist} from "@/services";
