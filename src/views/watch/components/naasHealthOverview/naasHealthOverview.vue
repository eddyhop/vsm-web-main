  ...
  <div class="vsrPandectMain">
    <div class="vsrPanCon">
      <dl class="hint">
        ...
        <div class="operate">
          <button class="add" v-control @click="goBgpRoutesPage()">
            {{ $t("Monit.showBgpRouts") }}
          </button>
        </div>
      </dl>
      ...
      <ul class="showList">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <li class="slItem lastItem">
              <h5 style="padding-bottom: 10px">{{ $t("Monit.BGPRoutes") }}</h5>
              <p class="flow">
                <b>{{ routerNum }}</b>
              </p>
            </li>
...
<script>
import bgpStatus from "./components/bgpStatus";
...
export default {
  ...
  methods: {
    ...
    getDevDataInit(params) {
      naasDeviceBaseInfo(params).then((res) => {
        if (res.code === 10000) {
