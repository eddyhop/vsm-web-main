  ...
  <div class="logical">
    ...
    <div class="tableView">
      ...
      >
        ...
        <el-table-column prop="clientName" :label="'Server '+$t('vsr1000.name')" show-overflow-tooltip :resizable="false"></el-table-column>
  ...
  methods: {
    ...
    removeVpnList() {
      if (this.multipleSelection.length) {
        ...
        let deviceId = JSON.parse(this.$route.query.devData).deviceId;
        let _this = this;
        var show51 = localStorage.getItem("show51");
        if(show51 == 'true'){
          ...
          this.$confirm(this.$t("vsr1000.confirmDel"+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue51\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>"), this.$t("vsr1000.hint"), {
          ...
          .then(() => {
            delReIpsec({
              ...
              this.lNumber = 1;
                localStorage.setItem("show51",localStorage.getItem("checkedValue51"));
              } else if (res.code !== 10000) {
                this.$message({
                  type: "error",
                  message: res.message
    ...
    remove(row) {
      let deviceId = JSON.parse(this.$route.query.devData).deviceId;
      let _this = this;
      var show52 = localStorage.getItem("show52");
      if(show52 == 'true'){
          delReIpsec({
            ids: [row.id],
            deviceId,
        ...
        this.$confirm(this.$t("vsr1000.confirmDel")+"<div style='left:10px;position:absolute;top:45px'><label><input onClick='localStorage.setItem(\"checkedValue52\",this.checked)' type='checkbox'> "+this.$t("doNotShowThisAgain")+"</label></div>", this.$t("vsr1000.hint"), {
          confirmButtonText: this.$t("vsr1000.OK"),
          cancelButtonText: this.$t("vsr1000.cancel"),
            dangerouslyUseHTMLString: true,
          type: "warning"
        ...
        .then(() => {
          delReIpsec({
            ...
            if (res.code === 10000) {
              ...
              this.lNumber = 1;
              localStorage.setItem("show52",localStorage.getItem("checkedValue52"));
            } else if (res.code !== 10000) {
              this.$message({
                type: "error",
                message: res.message
